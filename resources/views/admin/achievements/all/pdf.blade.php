...
<body>
    <div class="flex items-center my-1">
        <h1 class="text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl dark:text-white">
            <center>Daftar Prestasi Siswa (All)</center>
        </h1>
        ...
        <div class="table-responsive mt-2">
            <table class="w-full p-1 text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="border-y text-xs uppercase dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-3 py-3 shadow-md">
                            No.
                        </th>
                        <th scope="col" class="px-3 py-3 shadow-md">
                            Satuan Pendidikan
                    ...
                    @if (count($pdf) > 0)
                        ...
                        @foreach($pdf as $key=>$all)
                            <tr class="border-b text-black dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                                <th scope="row" class="px-3 py-3 font-medium whitespace-nowrap shadow-md">{{ $key+1 }}.</th>
                                <td class="px-3 py-3 shadow-md">{{ $all->name }}</td>
                                <td class="px-3 py-3 shadow-md">{{ $all->nama_siswa }}</td>
                                <td class="px-3 py-3 shadow-md">{{ $all->nama_lomba }}</td>
                                <td class="px-3 py-3 shadow-md">{{ $all->penyelenggara }}</td>
                                <td class="px-3 py-3 shadow-md">{{ $all->prestasi }}</td>
                                <td class="px-3 py-3 shadow-md">{{ Carbon\Carbon::parse($all->waktu)->isoFormat('D MMMM YYYY') }}</td>
                                <td class="px-3 py-3 shadow-md">
                                    @if($all->tingkat == 0)
                                        Kabupaten/Kota
