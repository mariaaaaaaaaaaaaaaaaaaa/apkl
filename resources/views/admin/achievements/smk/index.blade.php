@extends('layouts.adminapp')
    ...
    @endif
    <div class="p-2">
        <h3 class="font-bold uppercase pb-2">Daftar Prestasi Siswa : SMK</h3><hr>
        <div class="pt-2 relative overflow-x-auto shadow-md sm:rounded-lg">
            <div class="grid grid-cols-2 gap-6 p-4 bg-white">
                <div class=" ">
                    <form action="{{ route('admin.achievements.smk') }}" method="GET" class="flex items-center">   
                ...
                <div class=" ">
                    {{-- <a href="{{ route('admin.achievements.smk.pdf') }}" class="mr-6 absolute right-0 px-2 flex md:mb-0 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm py-3 dark:bg-blue-600 dark:hover:bg-blue-700">Cetak PDF</a> --}}
                    ...
                    <div id="dropdownLeftEnd" class="z-20 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownLeftEndButton">
                            <li><a href="{{ route('admin.achievements.smk.pdf') }}" class="block flex px-4 py-2 gap-3 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            ...
                            <li><a href="{{ route('admin.achievements.smk.excel') }}" class="block flex px-4 py-2 gap-3 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        ...
                        @if (count($acvmks) > 0)
                            @foreach($acvmks as $key=>$acvsmk)
                                <tr class="border-b text-black dark:bg-gray-800 dark:border-gray-700 hover:bg-yellow-200 dark:hover:bg-gray-600 dark:text-white">
                                    <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap">{{ $key+$acvmks->firstItem() }}.</th>
                                    <td class="px-6 py-4">{{ $acvsmk->user->name }}</td>
                                    <td class="px-6 py-4">{{ $acvsmk->nama_siswa }}</td>
                                    <td class="px-6 py-4">{{ $acvsmk->nama_lomba }}</td>
                                    <td class="px-6 py-4">{{ $acvsmk->penyelenggara }}</td>
                                    <td class="px-8 py-4">{{ $acvsmk->prestasi }}</td>
                                    <td class="px-6 py-4">{{ Carbon\Carbon::parse($acvsmk->waktu)->isoFormat('D MMMM YYYY') }}</td>
                                    <td class="px-6 py-4">
                                        @if($acvsmk->tingkat == 0)
                                            Kabupaten/Kota
                                        @elseif($acvsmk->tingkat == 1)
                                            Provinsi
                                        @elseif($acvsmk->tingkat == 2)
                                            Nasional
                                        @elseif($acvsmk->tingkat == 3)
                                            Internasional
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        @if($acvsmk->jenis_lomba == 1)
                                            Tidak Berjenjang
                                        @else
                                            Berjenjang
                                        @endif
                                    ...
                                    <td class="flex items-center px-6 py-4 space-x-3">
                                        <a href="{{ route('admin.users.edit', $acvsmk->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                        <button data-modal-target="defaultModal" data-item="{{ $acvsmk->id }}" data-modal-toggle="defaultModal" type="button" class="font-medium text-red-600 dark:text-red-500 hover:underline">Hapus</button>
                                        <!-- Main modal -->
                                                ...
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    ...
                                                    <div class="p-6 space-y-6">
                                                        <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                                            Apakah Anda yakin akan menghapus akun <strong class="uppercase">{{ $acvsmk->name }}</strong> secara permanen?
                                                        </p>
                                                    </div>
                                                    <!-- Modal footer -->
                                                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                                                        ...
                                                        <form action="{{ route('admin.users.destroy', $acvsmk->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
