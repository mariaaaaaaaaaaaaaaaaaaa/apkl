{{-- @extends('layouts.adminapp')
    ...
    <div class="font-bold border-b border-gray-200 dark:border-gray-700">
        <div class="text-base p-2">
            Edit Data Akun : {{ $users->name }}
        </div>
    </div>
    <div class="rounded-lg bg-white">
        <form action="{{ route('admin.users.update', ['id'=>$users->id]) }}" method="POST" id="edit_form" class="py-2">
            @csrf
            @method('PUT')
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div class="px-2">
                    <label for="role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">NPSN</label>
                    <input type="text" name="role" id="role" value="{{ $users->role }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : 12345678" required>
                </div>
                <div class="px-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Satuan Pendidikan</label>
                    <input type="text" name="name" id="name" value="{{ $users->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : SMA Negeri 1 Semarang" required>
                </div>
                <div class="px-2">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email Akun</label>
                    <input type="email" name="email" id="email" value="{{ $users->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : sman1smg@user.com" required>
                </div>  
                <div class="px-2">
                    <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                    <button data-modal-target="defaultModal" data-item="{{ $users->id }}" name="password" id="reset-user-btn" data-modal-toggle="defaultModal" class="uppercase bg-gray-50 border border-gray-300 text-red-500 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" type="button">Klik di sini untuk mereset password akun ini.</button>
                </div>
                <div class="px-2">
                    <label for="status_akun" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status Akun</label>
                    <select name="status_akun" id="status_akun" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option>Aktif</option>
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-6 space-y-6">
                            <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Apakah Anda yakin akan melakukan reset password pada akun <strong>{{ $users->name }}</strong>?
                            </p>
                        </div>
                        <!-- Modal footer -->
                        <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                            <button data-modal-hide="defaultModal" id="confirm-reset-btn" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                <a href="{{ route('admin.users.reset', ['id'=>$users->id]) }}" id="confirm-reset-link" class="text-white">
                                Reset</a>
                            </button>
