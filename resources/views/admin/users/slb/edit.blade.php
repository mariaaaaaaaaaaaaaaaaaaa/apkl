@extends('layouts.adminapp')
    ...
    <div class="font-bold border-b border-gray-200 dark:border-gray-700">
        <div class="text-base p-2">
            Edit Data Akun SLB : {{ $slbs->name }}
        </div>
    </div>
    <div class="rounded-lg bg-white">
        <form action="{{ route('admin.users.slb.update', ['id'=>$slbs->id]) }}" method="POST" id="edit_form" class="py-2">
            @csrf
            @method('PUT')
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div class="px-2">
                    <label for="npsn" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">NPSN</label>
                    <input type="text" name="npsn" id="npsn" value="{{ $slbs->npsn }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : 12345678" required>
                </div>
                <div class="px-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Satuan Pendidikan</label>
                    <input type="text" name="name" id="name" value="{{ $slbs->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : SLB Negeri 1 Semarang" required>
                </div>
                <div class="px-2">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email Akun</label>
                    <input type="email" name="email" id="email" value="{{ $slbs->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="contoh : slbn1smg@user.com" required>
                </div>
                <div class="px-2">
                ...
                {{-- <div class="px-2">
                    ...
                    <fieldset class="pl-2">
                        ...
                        <div class="flex items-center mb-2">
                            <input id="status" type="radio" name="status" value="0" {{ $slbs->status == 0 ? 'checked' : ''}} class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600">
                            <label for="status" class="block ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                Aktif
                            </label>
                        </div>
                        <div class="flex items-center mb-2">
                            <input id="status" type="radio" name="status" value="1" {{ $slbs->status == 1 ? 'checked' : ''}} class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600">
                            <label for="status" class="block ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                Tidak Aktif
                            </label>
                        </div>
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-6 space-y-6">
                            <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Apakah Anda yakin akan melakukan reset password pada akun <strong class="uppercase">{{ $slbs->name }}</strong>?
                            </p>
                        </div>
                        <!-- Modal footer -->
                        <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                            <button data-modal-hide="defaultModal" id="confirm-reset-btn" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                <a href="{{ route('admin.users.reset', ['id'=>$slbs->id]) }}" id="confirm-reset-link" class="text-white">
                                Reset</a>
                            </button>
