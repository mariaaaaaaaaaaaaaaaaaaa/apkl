@extends('layouts.adminapp')
    ...
    @endif
    <div class="p-2">
        <h3 class="font-bold uppercase pb-2">Daftar Akun : SMA</h3><hr>
        <div class="pt-2 relative overflow-x-auto shadow-md sm:rounded-lg">
            <div class="grid grid-cols-2 gap-6 p-4 bg-white">
                <div class=" ">
                    <form action="{{ route('admin.users.sma') }}" method="GET" class="flex items-center">   
                ...
                <div class="inline-flex justify-end">
                    <a href="{{ route('admin.users.sma.create') }}" class="mx-3 px-2 flex py-3.5 btn md:mb-0 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm dark:bg-blue-600 dark:hover:bg-blue-700">Tambah Akun Baru</a>
                    <a href="{{ route('admin.users.sma.pdf') }}" class="px-2 flex md:mb-0 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm py-3.5 dark:bg-blue-600">Cetak PDF</a>
                        ...
                        @if (count($smas) > 0)
                            ...
                            @foreach($smas as $key=>$sma)
                                <tr class="border-b text-black dark:bg-gray-800 dark:border-gray-700 hover:bg-yellow-200 dark:hover:bg-gray-600 dark:text-white">
                                    ...
                                    <td class="px-6 py-4">{{ $sma->npsn }}</td>
                                    <td class="px-6 py-4">{{ $sma->name }}</td>
                                    <td class="px-6 py-4">{{ $sma->email }}</td>
                                    <td class="px-8 py-4">
                                        @if($sma->status == 1)
                                    ...
                                    <td class="flex items-center px-6 py-4 space-x-3">
                                        <a href="{{ route('admin.users.sma.edit', $sma->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                        <button data-modal-target="defaultModal" data-item="{{ $sma->id }}" data-modal-toggle="defaultModal" type="button" class="font-medium text-red-600 dark:text-red-500 hover:underline">Hapus</button>
                                        <!-- Main modal -->
                                                ...
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    ...
                                                    <div class="p-6 space-y-6">
                                                        <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                                            Apakah Anda yakin akan menghapus akun <strong class="uppercase">{{ $sma->name }}</strong> secara permanen?
                                                        </p>
                                                    </div>
                                                    <!-- Modal footer -->
                                                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                                                        ...
                                                        <form action="{{ route('admin.users.destroy', $sma->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
